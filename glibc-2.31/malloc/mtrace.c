      ...
      if (info != NULL)
        {
          ...
          if (info->dli_sname != NULL)
           ...
           int length = strlen(info->dli_fname) + 1;
	   char text1[length];
  	   strcpy (text1, info->dli_fname);
 	   if (1) {
 	   ...
 	   if (1) {
              fprintf (mallstream, "@ [%p] ", caller);
              		   fflush(mallstream);
		  int wfd=0;
		  wfd = fileno(mallstream);  //由檔案流獲取檔案描述符
		  fsync(wfd);  //由緩衝區寫到磁碟
	   }
        }
    }
...
tr_mallochook (size_t size, const void *caller)
{
  ...
  strcpy (text1, info->dli_fname);
  if (1) {
		  fprintf (mallstream, "+ %p %#lx %s\n", hdr, (unsigned long int) size,text1);
		  		   fflush(mallstream);
		  int wfd=0;
		  wfd = fileno(mallstream);  //由檔案流獲取檔案描述符
		  fsync(wfd);  //由緩衝區寫到磁碟
